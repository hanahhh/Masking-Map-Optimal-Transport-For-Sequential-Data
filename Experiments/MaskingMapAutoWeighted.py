...
from MaskingMap.knn import knn_masking_map_auto_weighted_adjusted
...
def main(args):
    method = "masking_map_auto_weighted"
    with open("Config/AlgorithmAutoWeighted.json", "r") as file:
        algorithms = json.load(file)
    with open(args.data_path, "r") as file:
        data = json.load(file)
    for data_set in data[method]:
        ...
        for k in algorithms[method]["k"]:
            ...
            accuracy = knn_masking_map_auto_weighted_adjusted(
                X_train=X_train,
                X_test=X_test,
                y_train=y_train,
                y_test=y_test,
